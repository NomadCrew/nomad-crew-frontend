...
4. Implement `uploadDocument()`:
   ...
   ```
   - **CRITICAL:** Convert to ArrayBuffer using `await fetch(fileUri).then(res => res.arrayBuffer())`
   - Upload with `supabase.storage.from(STORAGE_BUCKET).upload()`
...
- Skip ArrayBuffer conversion (uploads will fail silently)
...
- No TypeScript errors
- ArrayBuffer pattern used (no FormData/Blob)
  </done>
</task>
...
- No TypeScript errors
- ArrayBuffer upload pattern used (per RESEARCH.md)
</success_criteria>
