...
export class WebSocketManager {
  ...
  /**
   * Get WebSocket URL without sensitive tokens in query params.
   * Token is passed via Sec-WebSocket-Protocol header instead (see WebSocketConnection).
   */
  private getWebSocketUrl(tripId: string): string {
    const base = API_CONFIG.BASE_URL.replace(/^http/, 'ws');
    // SECURITY: No longer passing token/apikey in URL - they go in headers
    return `${base}/v1/ws`;
  }
  ...
  public async connect(tripId: string, callbacks?: ConnectionCallbacks): Promise<void> {
    try {
      ...
      const wrappedCallbacks: ConnectionCallbacks = {
        ...
        onMessage: (messageData: any) => {
          ...
          if (notificationResult.success) {
            ...
            logger.debug(
              'WS',
              `Received valid Notification object, type: ${validatedNotification.type}`
            );
          ...
          if (typeof parsedData === 'object' && parsedData !== null && parsedData.type) {
            ...
            logger.warn(
              'WS',
              'Received message is not a valid Notification or recognizable ServerEvent:',
              parsedData
            );
          }
        },
      ...
      this.connection = new WebSocketConnection({
        ...
        token, // Passed to connection for Sec-WebSocket-Protocol header
        apiKey,
        tripId,
        userId: user.id,
        ...wrappedCallbacks,
  ...
  private handleLocationEvent(event: ServerEvent, tripId: string): void {
    if (event.type === 'LOCATION_UPDATED' && event.payload) {
      ...
      logger.debug(
        'WS',
        `User ${userId} ${isSharingEnabled ? 'enabled' : 'disabled'} location sharing`
      );
    }
  }
  ...
  private handleChatEvent(event: ServerEvent): void {
    ...
    if (eventType === 'CHAT_MESSAGE_SEND') {
      ...
      try {
        ...
        logger.debug(
          'WS',
          'Created MESSAGE_SENT event from CHAT_MESSAGE_SEND with message ID:',
          payload.messageId
        );
