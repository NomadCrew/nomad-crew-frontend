...
interface TodoItemProps {
  ...
  onDelete?: () => Promise<void>; // Optional - only present if user has permission
  textColor?: string;
  disabled?: boolean;
}
...
export const TodoItem = ({
  ...
  // Only enable swipe-to-delete if user has permission (onDelete is provided)
    ...
    .onEnd(() => {
      translateX.value =
        Math.abs(translateX.value) > SWIPE_THRESHOLD
          ...
          : withSpring(0);
      if (Math.abs(translateX.value) > SWIPE_THRESHOLD && onDelete) runOnJS(onDelete)();
    });
  ...
  const containerStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      borderLeftColor:
        todo.status === 'COMPLETE' ? theme.colors.status.success.background : 'rgba(0, 0, 0, 0.08)',
      borderTopWidth: 1,
      borderBottomWidth: 1,
      ...
      borderRightColor:
        isSwipingLeft && swipeProgress > 0.5
  ...
  const progressStyle = useAnimatedStyle(() => {
    'worklet';
    const currentContainerWidth = containerWidth.value;
    const width = interpolate(deleteProgress.value, [0, 1], [0, currentContainerWidth]);
  ...
  const onTextLayout = useCallback((event: { nativeEvent: { layout: { width: number } } }) => {
    textWidth.value = event.nativeEvent.layout.width;
  }, []);

  const onContainerLayout = useCallback((event: { nativeEvent: { layout: { width: number } } }) => {
    ...
    <GestureDetector gesture={Gesture.Race(panGesture, longPressGesture)}>
      <View style={styles.itemWrapper}>
        <Animated.View style={[styles.container, containerStyle]} onLayout={onContainerLayout}>
          ...
          >
            <Animated.Text ref={textRef} style={[styles.text, textStyle]} onLayout={onTextLayout}>
              {todo.text}
            </Animated.Text>
          </Animated.View>
          {/* Only show delete indicator if user has permission to delete */}
          {onDelete && (
